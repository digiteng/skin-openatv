	...
	def statusChanged(self):
		...
		match self.state:
			case self.RUNNING:
				...
				with open("/tmp/scan", "w") as f: f.write("{}".format(percentage))
				# rating_texts = {
					# (0, 0): "",
					# (1, 5): "",
					# (6, 10): "",
				...
				# for rating_range, rating_text in rating_texts.items():
					# if rating_range[0] <= int(percentage) <= rating_range[1]:
						# valll = rating_text
						# break
					# else:
						# valll = ""
				# self["circle_progress"].setText(valll)
				if percentage > 99:
